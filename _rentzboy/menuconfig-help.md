...
stm32f3discovery:nsh -> configura NSH en USART2 (PA2 y PA3). Tb incluye soporte para USB (CDC/ACM device), aunque no se utiliza para la console
stm32f3discovery:usbnsh -> configura NSH por el USER USB (VPC/CDC). Esta config. parece que no funciona sin modificar al menos board.h pues hay que asignar al USER USB el PA11 (USB_DM) y el PA12 (USB_DP). Posiblemente habrá que hacer más cambios.
...
Si no queremos compartir el ST-LINK para nsh shell y debugging, podemos utilizar el ST-LINK USB para debugging y el USER USB u Tx/Rx para la nsh shell utilizando un convertidor tipo CH340/Prolific. Problema: en los breakpoints o pausa al depurar el USART deja de funcionar pues el nucleo del µC está pausado.
...
CONFIG_DEV_CONSOLE is not set => Enable /dev/console (RTOS Features > Files and I/O). Si CONFIG_DEV_CONSOLE no está habilitado, el sistema utilizará el primer dispositivo serie que se registre (por ejemplo, /dev/ttyS0 o /dev/ttyS1) como la consola del sistema en nuttx/arch/arm/src/common/arm_internal.h. **LEER menuconfig ayuda \***
...
CONFIG_CONSOLE_SYSLOG => Redirige la Consola HACIA el Syslog (printf() -> /dev/console -> Syslog). NO CONFUNDIR con CONFIG_SYSLOG_CONSOLE
CONFIG_SYSLOG_CONSOLE => 1/3: es la opción más habitual. syslog() -> Consola (/dev/console)
CONFIG_SYSLOG_CHAR=y => 2/3: Log to a character device. Va de la mano con CONFIG_SYSLOG_DEVPATH
CONFIG_SYSLOG_DEVPATH="/dev/ttyS0" =>2/3: Full path to the system logging device. Va de la mano con CONFIG_SYSLOG_CHAR
CONFIG_RAMLOG_SYSLOG => 3/3: store logs in Circular Buffer memory for later retrieval with dmesg, without affecting the NSH console.
...
CONFIG_SYSLOG_CHARDEV => Enables support for a simple character driver at /dev/log. Es una interfaz de ENTRADA. Esta opción hace que el propio sistema syslog se registre a sí mismo como un dispositivo de caracteres, usualmente en /dev/kmsg, para que otros procesos puedan escribir en él. Todo lo que se escriba será inyectado en el sistema syslog como un nuevo mensaje de log. NO CONFUNDIR con CONFIG_SYSLOG_CHAR
CONFIG_SYSLOG_REGISTER=y => [auto] Register syslog channel support.
...
==============================================================================
Build setup > Debug options > CONFIG_DEBUG_SENSORS (info/warn/err)
Build setup > Debug options > CONFIG_DEBUG_I2C (info/warn/err)
System type > STM32 Peripheral Support > STM32_I2C1
Board Selection > (Board common logic) CONFIG_ARCH_BOARD_COMMON
RTOS Features > RTOS hooks > (Custom board late initialization) CONFIG_BOARD_LATE_INITIALIZE
Device Drivers > (I2C Driver Support) CONFIG_I2C
Device Drivers > I2C Driver Support > (I2C character driver) CONFIG_I2C_DRIVER
Device Drivers > Sensor Device Support > (STMicro LSM303DLHC 3D accele .... ) CONFIG_SENSORS_LSM303DLHC=y
Device Drivers > Sensor Device Support > CONFIG_SENSORS_LSM303DLHC_I2C=y
Device Drivers > Sensor Device Support > CONFIG_LSM303DLHC_I2C_FREQUENCY=400000
Application Configuration > Examples > (LSM303DLHC reader example) CONFIG_EXAMPLES_LSM303DLHC=y
